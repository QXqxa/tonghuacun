...
export default function Home(){
 ...
 useEffect(()=>{document.documentElement.classList.add('reveal-ready');const nodes=[...document.querySelectorAll<HTMLElement>('#album,#events,#honors,#sponsors')];nodes.forEach(node=>node.classList.add('reveal-section'));const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add('revealed');observer.unobserve(entry.target)}}),{threshold:.12,rootMargin:'0px 0px -8%'});nodes.forEach(node=>observer.observe(node));return()=>observer.disconnect()},[]);
 ...
 return <main>
 <header className="nav"><a className="brand" href="#home" aria-label="冒险岛怀旧服童话村首页"><Logo/>童话村</a><span className="nav-sub">冒险岛怀旧服</span><nav className="desktop-nav"><a href="#album">相册</a><a href="#events">大事记</a><a href="#honors">荣誉馆</a><a href="#sponsors">贡献榜</a><button onClick={()=>setUpload(true)}>上传照片 <Plus size={17}/></button></nav><Music/><button className="mobile-menu" aria-label={menu?'关闭菜单':'打开菜单'} aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>
 {menu&&<nav className="mobile-overlay"><a href="#album" onClick={()=>setMenu(false)}>我们的相册 ↗</a><a href="#events" onClick={()=>setMenu(false)}>童话村大事记 ↗</a><a href="#honors" onClick={()=>setMenu(false)}>百级荣誉馆 ↗</a><a href="#sponsors" onClick={()=>setMenu(false)}>家族贡献榜 ↗</a><button onClick={()=>{setMenu(false);setUpload(true)}}>上传照片 ＋</button></nav>}
