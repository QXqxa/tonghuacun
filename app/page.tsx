...
const samples: Photo[] = [{src:'/village-group.png',name:'童话村 · 冒险家大合影',note:'我们的相册'}];
...
export default function Home(){
 ...
 useEffect(()=>{let stopped=false;(async()=>{let cursor:string|null=null;const all:Photo[]=[];do{const r:Response=await fetch('/api/photos'+(cursor?'?cursor='+encodeURIComponent(cursor):''));if(!r.ok)throw new Error();const data=await r.json() as {photos:Photo[];cursor:string|null};all.push(...data.photos);cursor=data.cursor}while(cursor&&!stopped);if(!stopped)setPhotos([...all.reverse(),...samples.filter(p=>!all.some(item=>item.name===p.name))])})().catch(()=>{if(!stopped)setAlbumError('相册暂时未能加载，请刷新重试。')});return()=>{stopped=true}},[]);
 useEffect(()=>{
  ...
  const load=async(i:number)=>{const img=new Image();img.src=`/frames/${String(i).padStart(3,'0')}.webp`;await img.decode();if(stopped)return;const surface=document.createElement('canvas');surface.width=800;surface.height=450;surface.getContext('2d')!.drawImage(img,0,0,800,450);surfaces[i]=surface};
 ...
 const savePhotos=async()=>{setSaving(true);setError('');let completed=0;try{for(const photo of pending){const r=await fetch('/api/photos?name='+encodeURIComponent(photo.name),{method:'POST',headers:{Authorization:'Bearer '+uploadKey,'Content-Type':photo.file?.type??'application/octet-stream'},body:photo.file});const data=await r.json() as Photo & {error?:string};if(!r.ok)throw new Error(data.error??'上传失败，请重试');setPhotos(p=>[data,...p]);completed++;}setUpload(false);setTimeout(()=>document.getElementById('album')?.scrollIntoView({behavior:'smooth'}),150)}catch(e){setError(e instanceof Error?e.message:'网络异常，请重试')}finally{setPending(p=>p.slice(completed));setSaving(false)}};
 return <main>
